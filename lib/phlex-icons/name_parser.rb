...
module PhlexIcons
  class NameParser
    attr_reader :pack_name, :icon_name, :variant_name, :original_name, :klass
    ...
    def initialize(name)
      ...
      @pack_name, @icon_name, @variant_name = parse(@original_name)
    ...
    # @param pack_name [String] The kebab-case library name (e.g., "hero").
    ...
    def build_icon_class_name(pack_name, icon_name)
      library_module_name = pack_name.split('-').map(&:capitalize).join
    ...
    # @param pack_name [String] Original library name (for error message).
    ...
    def find_and_validate_icon_class(class_name, pack_name, icon_name)
      ...
      unless klass
        raise NameError,
              ...
              "Make sure the library ('#{pack_name}') and name ('#{icon_name}') " \
