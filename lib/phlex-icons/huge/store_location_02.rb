# frozen_string_literal: true

# rubocop:disable Layout/LineLength,Metrics/BlockLength
module PhlexIcons
  module Huge
    class StoreLocation02 < Base
      def stroke
        svg(
          **attrs,
          viewbox: '0 0 24 24',
          fill: 'none',
          xmlns: 'http://www.w3.org/2000/svg'
        ) do |s|
          s.path(
            d:
              'M2.97659 10.5146V15.009C2.97659 17.8339 2.97659 19.2463 3.85624 20.1239C4.73588 21.0015 6.15165 21.0015 8.98318 21.0015H12.9876',
            stroke: '#141B34',
            stroke_width: '1.5',
            stroke_linecap: 'round'
          )
          s.path(
            d: 'M6.98148 17.0066H10.9859',
            stroke: '#141B34',
            stroke_width: '1.5',
            stroke_linecap: 'round'
          )
          s.path(
            d:
              'M18.4941 13.5107C20.4292 13.5107 21.998 15.0464 21.998 16.9408C21.998 19.0836 19.8799 20.1371 18.8695 21.7433C18.6542 22.0857 18.3496 22.0857 18.1187 21.7433C17.0768 20.1981 14.9903 19.0389 14.9903 16.9408C14.9903 15.0464 16.559 13.5107 18.4941 13.5107Z',
            stroke: '#141B34',
            stroke_width: '1.5',
            stroke_linejoin: 'round'
          )
          s.path(
            d: 'M18.4942 17.0066H18.5032',
            stroke: '#141B34',
            stroke_width: '2',
            stroke_linecap: 'round',
            stroke_linejoin: 'round'
          )
          s.path(
            d:
              'M17.7957 2.00254L6.14986 2.03002C4.41169 1.94542 3.96603 3.2116 3.96603 3.83056C3.96603 4.38414 3.89058 5.19117 2.82527 6.70798C1.75996 8.22478 1.84001 8.67537 2.44074 9.72544C2.93931 10.5969 4.20744 10.9374 4.86865 10.9946C6.96886 11.0398 7.99068 9.32381 7.99068 8.1178C9.03254 11.1481 11.9956 11.1481 13.3158 10.8016C14.6386 10.4545 15.7717 9.2118 16.0391 8.1178C16.195 9.47735 16.6682 10.2707 18.0663 10.8158C19.5145 11.3805 20.7599 10.5174 21.3848 9.9642C22.0097 9.41096 22.4107 8.18278 21.2968 6.83288C20.5286 5.90195 20.2084 5.02494 20.1033 4.11599C20.0423 3.58931 19.9888 3.02336 19.5972 2.66323C19.0248 2.13691 18.2036 1.97722 17.7957 2.00254Z',
            stroke: '#141B34',
            stroke_width: '1.5',
            stroke_linecap: 'round',
            stroke_linejoin: 'round'
          )
        end
      end
    end
  end
end
# rubocop:enable Layout/LineLength,Metrics/BlockLength
